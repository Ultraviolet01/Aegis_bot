...
describe("PolicyParser — Multi-Asset Exit Support", () => {
  ...
  it("uses the cautious slippage ceiling in the deterministic fallback", async () => {
    const parser = new PolicyParser("");
    const parsed = await parser.parse("If SPYX drops more than 8% exit 75% to USDC cautiously");

    expect(parsed.drawdown_bps).toBe(800);
    expect(parsed.exit_bps).toBe(7500);
    expect(parsed.target_asset).toBe("USDC");
    expect(parsed.max_slippage_bps).toBe(30);
  });
});
